 ...
 */
export const linearSearch = <Value>(values: readonly Value[], matcher: SearchPredicate<Value> | Value): number => {
  const predicate: SearchPredicate<Value> =
    typeof matcher === "function" ? (matcher as SearchPredicate<Value>) : (value) => Object.is(value, matcher);
