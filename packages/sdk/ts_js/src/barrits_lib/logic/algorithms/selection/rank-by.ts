 ...
 */
export const rankBy = <Value>(values: readonly Value[], criteria: readonly OrderCriterion<Value>[]): RankedValue<Value>[] => {
  ...
  return sortedValues
    .map((value, index) => {
      ...
      const previousRank = compare(previousEntry, value) === 0 ? undefined : index + 1;
