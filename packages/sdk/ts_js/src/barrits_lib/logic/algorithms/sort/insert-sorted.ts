 ...
 */
export const insertSorted = <Value>(values: readonly Value[], value: Value, compare: CompareFunction<Value> = defaultCompare): Value[] => {
  const insertionIndex = lowerBound(values, value, compare);

  return [...values.slice(0, insertionIndex), value, ...values.slice(insertionIndex)];
};
