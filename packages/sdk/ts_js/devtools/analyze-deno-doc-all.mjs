/**
 * analyze-deno-doc-all.mjs
 * Merges `deno doc --json` outputs from all JSR entry points and counts
 * documented vs undocumented exported symbols (incl. nested members),
 * mirroring JSR's "Has docs for most symbols" scoring.
 * @module
 */
...
for (const f of files) {
  ...
  for (const k of Object.keys(nodes)) {
    if (!merged.nodes[k]) merged.nodes[k] = nodes[k];
    else merged.nodes[k].symbols.push(...(nodes[k].symbols ?? []));
  }
}
...
// JSR requires a real description, not just tags.
...
function record(node, path) {
  ...
  const d = hasDoc(node);
  results.push({ kind: node.kind ?? "?", name, path, documented: d, loc: locOf(node) });
  total++; if (d) documented++;
}
function walkDefMembers(def, path) {
  ...
  for (const arr of ["methods", "properties", "getters", "setters", "indexSignatures", "callSignatures", "enum", "enums"]) {
    if (Array.isArray(def[arr])) for (const m of def[arr]) record(m, path);
  }
  if (Array.isArray(def.nodes)) for (const m of def.nodes) walkSymbol(m, path);
}
function walkSymbol(sym, path) {
  record(sym, path);
  const decl = sym.declarations && sym.declarations[0];
  walkDefMembers(decl && decl.def, `${path}/${sym.name ?? ""}`);
  if (Array.isArray(sym.symbols)) for (const m of sym.symbols) walkSymbol(m, `${path}/${sym.name ?? ""}`);
}

for (const fileUrl of Object.keys(merged.nodes)) {
  for (const sym of (merged.nodes[fileUrl].symbols ?? [])) walkSymbol(sym, "");
}

const pct = total > 0 ? ((documented / total) * 100).toFixed(1) : "N/A";
console.log(`Total exported symbols (all entry points, incl. members): ${total}`);
