...
describe("example-svelte", () => {
  ...
  it("barrits: duplicar function works correctly", async () => {
    const { duplicar } = await import(url("src/barrits/logic/duplicar.ts"));
  ...
  it("barrits: vite.config.ts loads with barrits plugin", async () => {
    ...
    ok(plugins.length >= 2, "Expected at least 2 plugins (svelte + barritsVitePlugin)");
  });
