...
import { generateOpenApiSchema, type BarritsOpenApiOptions } from "../src/barrits/schema/openapi";
...
describe("generateOpenApiSchema", () => {
  const toSchema = (manifest: BarritsBuildManifest, options?: BarritsOpenApiOptions) =>
    generateOpenApiSchema(manifest, options) as Record<string, unknown>;
  ...
  it("uses default title, version, and description", () => {
    const schema = toSchema(makeManifest([])) as Record<string, Record<string, string>>;
