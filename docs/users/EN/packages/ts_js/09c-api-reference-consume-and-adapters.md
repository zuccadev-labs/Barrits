...
Parses a raw JSON string into a validated `BarritsBuildManifest` without touching the filesystem. Use it when you already hold the manifest payload (for example, from an in-memory cache or a non-file source).
...
Parses a raw JSON string into a validated `BarritsWatchSnapshot`.

### `createBuildManifestSummary(manifest)`

Builds a compact, UI-ready summary from a parsed (or injected) build manifest. This is the same helper the bundler plugins use internally to expose `virtual:barrits/manifest` to frontend apps.
...
Appears in: `examples/bundlers/*/*-manifest-entry.mjs` (via `virtual:barrits/manifest`).

### `createWatchSnapshotSummary(snapshot)`

Builds a compact summary from a parsed `BarritsWatchSnapshot`.

### `createLanguageToolSnapshot(snapshot)`

Builds a detailed, editor-oriented snapshot from a parsed `BarritsWatchSnapshot`.
