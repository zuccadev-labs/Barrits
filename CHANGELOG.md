...
- **Hito 3 · fix(manifest)**: el checksum SHA-256 del manifiesto cubre ahora todo el contenido determinista (antes solo nombres y contadores); nuevas `verifyBuildManifest`, `assertBuildManifestIntegrity` y `computeBuildManifestChecksum`; `deterministicStringify` ordena por code unit (reproducible entre runtimes); `engines.node >= 20`. README y ARCHITECTURE.md describen el mecanismo real.
- **Hito 4 · fix(lib)**: corrección de `defaultCompare`, `minBy`/`maxBy`, `slugify`, `truncate`, `capitalizeWords`, EMA, `maxFlow`, `murmurHash3`, `isIsoDate`, `withTimeout` (`TimeoutError`), `toRelativeTime`; `quickSort` iterativo, heap binario compartido (`topK`, Dijkstra), grafos y ventanas en tiempo lineal, normalización de tamaños frente a `NaN`; la superficie `logic` se deriva de la librería (strings, validation, comparadores y familias ahora visibles desde la raíz). 88 tests nuevos (unitarios + property-based).
- **Hito 5a · style(sdk)**: formato Prettier aplicado a `src/`, `adapters/` y `tests/` del SDK (sin cambios funcionales); `npm run format` pasa a ser un gate.
- **Hito 5b · ci**: CI ejecuta los tests de los diez ejemplos, añade comprobación de formato y `concurrency`; workflows de seguridad fusionados; release determinista (`npm ci`, `deno publish`, permisos por job); un único `.github/CODEOWNERS` real; Dependabot cubre Cargo; hooks de pre-commit versionados sin husky/lint-staged (`scripts/prepare-hooks.mjs`); `CONTRIBUTING.md` y ADR 0001 alineados con el flujo real.
- **Hito 6 · refactor(cli)**: núcleo de CLI único (`runCli` sobre el trait `CliRuntimeIo`) con adaptadores Node/Bun y Deno reducidos a primitivas de E/S; un solo adaptador de filesystem y un solo juego de lectores de manifiestos; completions derivadas de las constantes del parser (`--file-kind` y `compdef` corregidos); `discoveryRoots` y `debugCommands` de la configuración por fin operativos; caché AST acotada.
- **Hito 7 · refactor(ast)**: los bloques JSDoc (`@barrits-trait`, `@barrits-path`) se leen del nodo AST al que el parser los asocia (fin de las fugas entre declaraciones); `export class` entra en el manifiesto con `kind: "class"`; `export * from "./x.js"` resuelve a `x.ts` y los especificadores sin extensión prueban todas las extensiones e `index.<ext>`; el crawler omite `*.d.ts`, tests y directorios generados y reconoce `index.<ext>` con cualquier extensión.
- **Hito 8 · refactor(config)**: vocabulario único de configuración (`BARRITS_RUNTIME_KINDS`, `BARRITS_WATCH_MODES`, `TRAIT_CONFLICT_STRATEGIES = throw/left/right`) con validación real y mapeo de las grafías heredadas `error/override/merge`; `ResolvedBarritsPackageOptions`/`ResolvedBarritsConfig` unificados; `namespace` validado (identificador, nunca `brt`/`config`); `createBarrits().composeTraits` consume por fin `traitConflictStrategy`; descriptores concretos aceptados por `composeTraitDescriptors` sin `as any`; Bun detectado como host; parser JSDoc que ya no captura la línea siguiente en tags sin valor y nombre de trait por literal `name`/binding; docs 09d/12, `ARCHITECTURE.md` y ejemplos Deno alineados.
...
- **Demo namespaced espejada en ejemplos de bundlers (opción A)**: `examples/bundlers/scripts/namespaced-demo.ts` ejecuta `createBarrits()` y demuestra `barrits.logic.orderBy` (y el alias `brt`), sin tocar los entries de bundler (`*-manifest-entry.mjs`), que son superficies de consumo de manifiesto vía `virtual:barrits/manifest`. Se añadió el script `demo:namespaced` en `bundlers/package.json`. El demo fue validado en runtime: namespace resuelto a `barrits` y `orderBy` ordena correctamente (tests de bundlers 11/11 en verde).
- **Tauri documenta por qué no usa `createBarrits()`**: `examples/example-tauri/README.md` ahora explica que el renderer evita la API namespaced runtime por diseño de seguridad (no toca el filesystem ni hace discovery) y porque el proyecto no tiene dominio `barrits/`; consume vía `@zuccadev-labs/barrits/consume` con `readTextFile` inyectado. Se añadió un comentario aclaratorio en `src/main.ts`.
...
### Security
- **4 advisories de severidad alta resueltos (`npm audit`)**: el gate `npm audit --omit=dev --audit-level=high` del workflow Security bloqueaba la promoción. Las cuatro eran dependencias transitivas, por lo que la remediación es solo de lockfile y ningún manifiesto cambió: `nanoid` 3.3.15 → 3.3.19 (GHSA-28wg-ghj8-5hjv, GHSA-2v37-7h3g-55p8), `postcss` 8.5.16 → 8.5.28 (GHSA-fxqj-rqcc-2cmp, GHSA-r28c-9q8g-f849) y `react-router`/`react-router-dom` 7.18.1 → 7.18.3 (GHSA-qwww-vcr4-c8h2). Grafo de producción y grafo completo quedan en 0 vulnerabilidades.

### Release engineering
- **`jsr.json` desalineado bloqueaba el tag**: el manifiesto JSR del SDK seguía declarando `0.2.4`, la última versión publicada, mientras ambos `package.json` ya estaban en `0.3.0`. El job `prepare` del workflow de release compara los tres campos contra el tag y aborta si alguno difiere, por lo que un tag `v0.3.0` habría fallado con `jsr.json version '0.2.4' does not match tag version '0.3.0'`. Los tres campos y el lockfile leen ahora `0.3.0`.
- **Rutas literales a `node_modules` eliminadas de todos los scripts**: diez manifiestos y dos archivos fuente invocaban `tsx` (y `tsup`, `tsc`, `typedoc`) por una ruta relativa a un `node_modules` concreto. Cuál es la correcta depende de si npm decide hoistear el paquete a la raíz o dejarlo en el workspace, y esa decisión cambia con la forma del árbol de dependencias; el historial de este CHANGELOG muestra la ruta invertida dos veces en ambos sentidos. Los scripts llaman ahora a los binarios por nombre, apoyándose en que npm pone en PATH tanto el `.bin` del workspace como el de la raíz. Los dos archivos que lanzan `tsx` como proceso hijo y no pueden usar PATH lo resuelven vía `createRequire(...).resolve("tsx/package.json")`. Corrige el fallo de CI `Cannot find module .../node_modules/tsx/dist/cli.mjs` en ubuntu y windows.
- **Lockfile reconstruido desde una resolución limpia**: el job `sbom` del workflow Security, que solo corre en push a `main`, falla cuando `npm ls --json --long --all` encuentra un paquete instalado fuera de un rango declarado (`ELSPROBLEMS`). El lockfile del merge se había producido reconciliando el de `dev` contra los manifiestos fusionados en lugar de resolver desde cero, y la reconciliación conservadora dejó `vue` deduplicado en 3.5.39 frente al `^3.5.42` de `example-vue`, y `@vitejs/plugin-react` en 6.0.3 frente al `^6.1.1` de `example-react`. Ambos árboles son instalables y ambos los rechaza `npm ls`. Tras la resolución limpia el comando sale con código 0.

### Dependencies
- **Dependabot reapuntado a `dev`**: `.github/dependabot.yml` no declaraba `target-branch`, así que Dependabot usaba la rama por defecto, `main`. Los commits de dependencias aterrizaban directamente en `main` mientras el trabajo de producto se acumulaba en `dev`, las ramas divergían y cada promoción tenía que reconciliar la diferencia a mano. Los tres ecosistemas apuntan ahora a `dev`, que es el flujo que el proyecto sigue realmente. Además se agrupa `@stryker-mutator/*`, que se versiona en lockstep y llegaba como tres PRs individualmente no mergeables, y se ignora `typescript` en `>=7.0.0` con el motivo y la condición de revisión documentados en línea.
- **`@stryker-mutator/core`, `tap-runner` y `typescript-checker` 9.6.1 → 10.0.0**: salto mayor validado con `stryker run --dryRunOnly`, que instrumentó 12 archivos con 2083 mutantes y completó su corrida inicial. Requiere Node >= 22, que es lo que usa CI, y no aparece en ningún workflow, por lo que no puede alcanzar el artefacto publicado.
- **`tauri` 2.10.3 → 2.11.5 y `tauri-build` 2.5.6 → 2.6.3**: `Cargo.toml` ya permitía ambas bajo sus rangos existentes, así que son actualizaciones únicamente de `Cargo.lock`. Aplicadas con `cargo update -p tauri -p tauri-build` en lugar de transcribiendo el diff, y verificadas con `cargo metadata --locked`.
- **`typescript` 6.0.3 → 7.0.2 rechazado**: TypeScript 7 es la reescritura nativa. Su export raíz es `./lib/version.cjs`, un string de versión, y la API del compilador se movió a subpaths que el propio paquete marca como `unstable`, por lo que la superficie clásica sobre la que está construido el descubrimiento de traits (`createSourceFile`, `ScriptTarget`, `VariableStatement`, `getJSDocCommentsAndTags`) no es alcanzable desde la raíz del paquete. El typecheck falla con TS2694 y TS2339 en los tests de AST. Se acotó además el rango de `peerDependencies` de `>=5.0.0` a `>=5.0.0 <7.0.0`: al no tener techo, estaba metiendo 7.0.2 como peer auto-instalado que ensombrecía el 6.0.3 hoisteado, y prometía un soporte que el paquete no tiene.
